...
SYSTEM_PROMPT = """You are an expert in information retrieval and contrastive training for text embedding models.
Generate exactly 7 high-quality hard negatives for the supplied query-positive pair.

First infer the exact information need, why the positive satisfies it, and the critical requirements. A hard negative must be strongly related, lexically and semantically similar, natural and corpus-like, and have the same style as the positive, but it must NOT satisfy the actual information need or contain the correct answer in another form. Whenever possible, violate only one critical requirement.

Length requirement (mandatory): the user provides the positive document's word count. Each hard negative's `text` must contain exactly that many whitespace-delimited words. Count the words in every completed negative before returning it, then expand or shorten it as needed without changing its assigned strategy or accidentally answering the query. Do not use padding, repeated sentences, filler, or metadata to reach the target count.
...
def system_prompt(enable_thinking: bool) -> str:
    """Build the prompt for reasoning-enabled or reasoning-disabled model runs."""
    return SYSTEM_PROMPT if enable_thinking else "/no_think\n" + SYSTEM_PROMPT
...
def generate(row: tuple[int, str, str], args: argparse.Namespace) -> dict[str, Any]:
    ...
    user_prompt = (
        f"Query:\n{query}\n\n"
        f"Positive document:\n{positive}\n\n"
        f"Positive word count (whitespace-delimited): {positive_word_count}\n"
        "Every hard negative must have exactly this word count."
    )
...
def main() -> None:
    ...
    thinking_group.add_argument(
        ...
        action="store_true",
        help="Enable the model's reasoning/thinking mode.",
    )
    thinking_group.add_argument(
        "--no-thinking",
        dest="enable_thinking",
        action="store_false",
        help="Disable the model's reasoning/thinking mode (default).",
    )
